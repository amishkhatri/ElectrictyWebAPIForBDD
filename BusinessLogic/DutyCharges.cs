    ...
    public partial class DutyCharges : EnergyCharges
    {
...

        {
            ...
            else if (units >= (int)slab2range.min && units <= (int)slab2range.max)
            {
                ...
                SlabWiseCharges = (50 * GetDutySlabRate((int)ElectricitySlabs.slab1)) + ((units - 50) * GetDutySlabRate((int)ElectricitySlabs.slab2));

            }
            else if (units >= (int)slab3range.min && units <= (int)slab3range.max)
            {

                SlabWiseCharges = (50 * GetDutySlabRate((int)ElectricitySlabs.slab1)) + (100 * GetDutySlabRate((int)ElectricitySlabs.slab2)) + ((units - 150) * GetDutySlabRate((int)ElectricitySlabs.slab3));
            }
